...
pub fn auth_ngrok(ngrok_authtoken: String) {
    // Authenticate ngrok
    Command::new("./ngrok".to_string())
    .arg("authtoken")
    .arg(ngrok_authtoken)
    .output()
    .expect("The authtoken to be valid.");
}
...
impl Builder {
    ...
    /// Set the tunnel auth
    pub fn auth(&mut self, auth: String) -> Self {
        self.auth = Some(auth);
        self.clone()
    }
    ...
    pub fn run(self) -> Result<Tunnel, io::Error> {
        ...
        let auth = self
            .auth
            .ok_or(Error::BuilderError(".auth(auth) should have been set"))?;

        let host = self
            .host
            .ok_or(Error::BuilderError(".host(host) should have been set"))?;
        ...
        let (tunnel_http, tunnel_https) = {
            loop {
                let tunnels = find_tunnels(port.clone());
                if tunnels.is_ok() {
                    break tunnels;
                }
...
fn find_tunnels(port: String) -> Result<(url::Url, url::Url), io::Error> {
    ...
    let tunnel_http = find_tunnel_url("http://", port.clone(), tunnels)?;
